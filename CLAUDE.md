...
**引擎选择** (`engine`)：任务级开关，前端引擎下拉，默认 `mineru`。三引擎路由：
- `mineru`（MinerU，默认）→ `mineru_client.process_mineru()`，ACADxPDF 式异步三步
- `vl16`（PaddleOCR-VL-1.6）→ `ocr_client` 现有流程，受 `high_precision` 影响
- `ppocrv6`（PP-OCRv6）→ `ocr_client.recognize_ppocrv6()`，零幻觉文字行
